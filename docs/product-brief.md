...
M0aの仮UIは、状態を示さないすべての要素をグレースケールにする。有彩色は選択、進行、処理中、
承認、未確認、警告、エラーなどの状態表示だけに使い、装飾や一般的な強調には使わない。
状態は色だけに依存せず、ラベル、アイコン、形、位置のいずれかを併用する。
...
- **2026-07-18 ユーザー指摘/判断**: 仮UIの視覚ルールを、状態以外はグレースケールとする。
  Before: M0aではブランド表現や最終配色を固定しないとしていたが、仮UIの色の使用範囲は未定義だった。
  After: 背景、文字、罫線、アイコン、図形、装飾はグレースケールとし、有彩色は明示的な状態表示だけに
  限定する。状態は色だけに依存せず、ラベル、アイコン、形、位置を併用する。情報設計と操作の評価を
  装飾やブランド配色から切り離しつつ、状態の違いは一目で追えるようにするため。
